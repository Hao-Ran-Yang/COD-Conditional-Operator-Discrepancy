...
parser.add_argument('--tradeoff_kgw', type=float, default=1e-3, help='tradeoff for kgw dist')
parser.add_argument('--tradeoff_cod', type=float, default=2e-4, help='tradeoff for cod dist')

parser.add_argument('--num_iter', type=int, default=20000, help='total number of iter')
...
class label_transforms:
    def __init__(self, dset_name='dSprites'):
        """
...
class Model_Regression(nn.Module):
    def __init__(self, dim=512):
        ...
        self.regressor = nn.Linear(dim, 3)
...
for exp_iter in {1}: #range(1,10):
    # set dataset
    batch_size = {"source_train": args.batch, "target_train": args.batch,"target_test": args.batch}
    c="color.txt"
    n="noisy.txt"
    s="scream.txt"
